...
import { getTiranaPlace, type TiranaPlace } from "./tiranaPlaces";
...
type Suggestion = {
  ...
  streetSteps: {
    title: string;
    detail: string;
    icon: "start" | "straight" | "arrive";
  }[];
  routeKind: "balanced" | "scenic" | "fastest";
  tags: string[];
...
type RouteVariant = {
  ...
  title: string;
  description: string;
  routeKind: Suggestion["routeKind"];
  confidence: number;
  tags: string[];
  streetSteps: Suggestion["streetSteps"];
};

function midpoint(a: [number, number], b: [number, number], latOffset = 0, lngOffset = 0): [number, number] {
  return [((a[0] + b[0]) / 2) + latOffset, ((a[1] + b[1]) / 2) + lngOffset];
}

function routeCoordsBetweenPlaces(fromPlace: TiranaPlace | undefined, toPlace: TiranaPlace | undefined, moodId: string, routeKind: Suggestion["routeKind"]): [number, number][] {
  if (!fromPlace || !toPlace) {
    return routeCoordsForMood(moodId, routeKind);
  }

  const moodBias: Record<string, [number, number]> = {
    calm: [0.0018, -0.0012],
    happy: [0.0011, 0.0015],
  ...
  const kindBias: Record<Suggestion["routeKind"], [number, number]> = {
...
function streetStepsBetweenPlaces(fromPlace: TiranaPlace | undefined, toPlace: TiranaPlace | undefined, variant: RouteVariant): Suggestion["streetSteps"] {
  if (!fromPlace || !toPlace) {
    return variant.streetSteps;
  }

  const moodLineByKind: Record<Suggestion["routeKind"], string> = {
...
function estimateDurationMinutes(transport: string, moodId: string): number {
  const baseByTransport: Record<string, number> = {
    walking: 28,
    bike: 18,
    car: 12,
    transit: 22
...
function buildGoogleMapsLink(from: string, to: string, transport: string): string {
...
function routeCoordsForMood(moodId: string, routeKind: Suggestion["routeKind"]): [number, number][] {
  const seeds: Record<string, Record<Suggestion["routeKind"], [number, number][]>> = {
    calm: {
...
function routeVariantsForMood(moodId: string, moodLabel: string, routeStyle: string): RouteVariant[] {
  const variants: Record<string, RouteVariant[]> = {
    calm: [
      {
        id: "parkline-calm",
...
export function generateSuggestions({ from, to, moodId, transport, avoid }: SuggestionRequest): Suggestion[] {
  const mood = getMoodById(moodId);
  if (!mood) return [];
  const fromPlace = getTiranaPlace(from);
  const toPlace = getTiranaPlace(to);
  ...
  const durationByKind: Record<Suggestion["routeKind"], number> = {
    balanced: firstDuration,
    scenic: secondDuration,
    fastest: thirdDuration
  };
  const stressByKind: Record<Suggestion["routeKind"], number> = {
    balanced: moodId === "stressed" ? 2 : 4,
    scenic: 3,
    fastest: moodId === "focused" ? 4 : 5
  };
  const elevationByKind: Record<Suggestion["routeKind"], number> = {
    balanced: 24,
    scenic: 38,
    fastest: 18
  };
  const safetyByKind: Record<Suggestion["routeKind"], number> = {
    balanced: 86,
    scenic: 82,
    fastest: 76
  };
  const noiseByKind: Record<Suggestion["routeKind"], Suggestion["noiseLevel"]> = {
    balanced: "Medium",
    scenic: "Low",
    fastest: moodId === "happy" || moodId === "energetic" ? "High" : "Medium"
  };

  const suggestions: Suggestion[] = routeVariantsForMood(moodId, mood.label, mood.routeStyle).map((variant) => ({
    id: variant.id,
    title: variant.title,
  ...
  return suggestions.filter((item) => {
    if (avoidZones.highStress && item.stressScore > 3) return false;
    if (avoidZones.crowded && item.tags.includes("crowded")) return false;
    if (avoidZones.noisy && item.tags.includes("noisy")) return false;
    return true;
  });
}
